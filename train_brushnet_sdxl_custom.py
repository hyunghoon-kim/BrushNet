...
def log_validation(vae, unet, brushnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    if not is_final_validation:
        ...
        if args.pretrained_vae_model_name_or_path is not None:
            vae = AutoencoderKL.from_pretrained(args.pretrained_vae_model_name_or_path, torch_dtype=weight_dtype, variant=args.variant)
        else:
            vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path, subfolder="vae", torch_dtype=weight_dtype, variant=args.variant) # 😀 FIX
    ...
    image_logs = []
    # inference_ctx = contextlib.nullcontext() if is_final_validation else torch.autocast("cuda") # FIX! prevent NaN error
    inference_ctx = contextlib.nullcontext() # 😀 FIX
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Number of training samples to be used for computing training steps (e.g., 5000)."
    )
...
class MyWebDataset():
    def __init__(self,resolution,text_encoders, tokenizers, random_mask):
    ...
    def __call__(self,examples):
        ...
        for example in examples:
            ...
            ## 😀 ADD, for check
            # tmp_name = random.randint(0, 20)
            # cv2.imwrite(f"{tmp_name}_image.jpg", image)
            # cv2.imwrite(f"{tmp_name}_masked_image.jpg", masked_image)
            # cv2.imwrite(f"{tmp_name}_mask.jpg", mask*255)
...
def main(args):
    ...
    if args.brushnet_model_name_or_path:
        ...
        # brushnet = BrushNetModel.from_pretrained("/workspace/BrushNet/data/ckpt/segmentation_mask_brushnet_ckpt_sdxl_v1")
    else:
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        shuffle=False,
        collate_fn=MyWebDataset(resolution = args.resolution, text_encoders = text_encoders, tokenizers = tokenizers, random_mask=args.random_mask),
    ...
    # num_update_steps_per_epoch = math.ceil(train_dataloader_len / args.gradient_accumulation_steps) # 😀 REPLACE
    num_update_steps_per_epoch = math.ceil(train_dataloader_len / (accelerator.num_processes * args.gradient_accumulation_steps)) # 😀 REPLACE

    if overrode_max_train_steps:
        args.max_train_steps = args.num_train_epochs * num_update_steps_per_epoch
    # Afterwards we recalculate our number of training epochs
    args.num_train_epochs = math.ceil(args.max_train_steps / num_update_steps_per_epoch)
    print(f"😀 num_update_steps_per_epoch={num_update_steps_per_epoch}") if accelerator.is_main_process else ... # 😀 ADD PRINT
    print(f"😀 num_train_epochs={args.num_train_epochs}") if accelerator.is_main_process else ... # 😀 ADD PRINT
    print(f"😀 args.max_train_steps={args.max_train_steps}") if accelerator.is_main_process else ... # 😀 ADD PRINT
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        print(f"during train validation! 😀 epoch: {epoch+1}, global_step: {global_step}", flush=True)
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt is not None:
            print("final validation! 😀", flush=True)
            image_logs = log_validation(
                vae=None,
                unet=None,
                brushnet=None,
